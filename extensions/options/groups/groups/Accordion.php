...
final class Accordion extends BaseGroup {
	...
	public function enqueueOptionScripts( array $group ) : void {
		...
		if ( Environment::isDevelopment() ) {
			wp_register_style( DHT_PREFIX_CSS . '-accordion-group', DHT_ASSETS_URI . 'dist/css/accordion.css', array(), dht()->manifest->get( 'version' ) );
			wp_enqueue_style( DHT_PREFIX_CSS . '-accordion-group' );
			
			wp_enqueue_script( DHT_PREFIX_JS . '-accordion-group', DHT_ASSETS_URI . 'dist/js/accordion.js', array( 'jquery' ), dht()->manifest->get( 'version' ), true );
		}
	}
