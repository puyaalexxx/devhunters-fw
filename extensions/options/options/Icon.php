...
final class Icon extends BaseOption {
    ...
    public function enqueueOptionScripts( array $option ) : void {
        ...
        wp_register_style( DHT_PREFIX . '-icon-option', DHT_ASSETS_URI . 'styles/css/extensions/options/icon-style.css', array(), fw()->manifest->get( 'version' ) );
        wp_enqueue_style( DHT_PREFIX . '-icon-option' );
        
        //custom option script
        wp_enqueue_script( DHT_PREFIX . '-icon-option', DHT_ASSETS_URI . 'scripts/js/extensions/options/icon-script.js', array( 'jquery' ), fw()->manifest->get( 'version' ), true );
    ...
    public function saveValue( array $option, mixed $option_value ) : mixed {
        ...
        return (array)json_decode( stripslashes( $option_value ) );
    }
