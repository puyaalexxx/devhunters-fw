...
final class UploadImage extends BaseOption {
    ...
    public function enqueueOptionScripts( array $option ) : void {
        ...
        wp_register_style( DHT_PREFIX . '-upload-image-option', DHT_ASSETS_URI . 'styles/css/extensions/options/upload-image-style.css', array(), fw()->manifest->get( 'version' ) );
        wp_enqueue_style( DHT_PREFIX . '-upload-image-option' );
        
        wp_enqueue_script( DHT_PREFIX . '-upload-image-option', DHT_ASSETS_URI . 'scripts/js/extensions/options/upload-image-script.js', array( 'jquery', 'media-editor' ), fw()->manifest->get( 'version' ), true );
    }
